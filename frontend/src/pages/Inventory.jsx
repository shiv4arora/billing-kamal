...
export default function Inventory() {
  ...
  useEffect(() => {
    if (!ledgerModal) { setProductLedger([]); return; }
    api(`/stock-ledger?productId=${ledgerModal.id}`)
      .then(data => setProductLedger((data || []).sort((a, b) => new Date(b.date) - new Date(a.date))))
      .catch(() => setProductLedger([]));
  }, [ledgerModal]);
  ...
  const handleAdjust = async () => {
    ...
    const qty = +adjQty;
    try {
      const updated = await api(`/products/${adjModal.id}/stock`, {
        method: 'PATCH',
        body: { delta: qty, movementType: 'adjustment', notes: adjNotes },
      });
      updateStockLocal(adjModal.id, updated.currentStock);
