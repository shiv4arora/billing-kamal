...
router.post('/', async (req, res, next) => {
  try {
    ...
    // Guard: cannot return more than was sold on the original invoice
    // (accounting for quantities already returned against it).
    if (originalInvoiceId) {
      const orig = await prisma.saleInvoice.findUnique({ where: { id: originalInvoiceId } });
      if (orig) {
        ...
        const priorReturns = await prisma.saleReturn.findMany({ where: { originalInvoiceId } });
