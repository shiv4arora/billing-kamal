...
import { Button, Card, Badge, useToast, Toast } from '../components/ui';
...
export default function Reminders() {
  ...
  const { logReminderSent, getReminderEntry } = useReminderLog();
  ...
  const allPending = useMemo(
    () => computePendingReminders(saleInvoices, { ...reminderSettings, enabled: true }, getReminderEntry),
    [saleInvoices, reminderSettings, getReminderEntry]
  );
  ...
  const sendOne = (r) => {
    const customer = getCustomer(r.inv.id) || { phone: '', name: r.inv.customerName };
    ...
    logReminderSent(r.inv.id, r.offset, { customerName: r.inv.customerName, invoiceNumber: r.inv.invoiceNumber });
  ...
  const sendAll = () => {
    ...
    setSending(true);
    // Dedupe by invoice — send once per invoice
    const deduped = {};
    ...
    list.forEach((r, i) => {
      setTimeout(() => {
        ...
        window.open(url, '_blank');
        logReminderSent(r.inv.id, r.offset, { customerName: r.inv.customerName, invoiceNumber: r.inv.invoiceNumber });
        if (i === list.length - 1) { setSending(false); toast.success(`${list.length} reminder(s) sent`); }
      }, i * 600);
    });
  };
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {rows.map(r => {
                  ...
                  // Find the most recent log entry for this invoice across all offsets
                  const reminderOffsets = (reminderSettings.schedule || [0, 3, 7]);
                  const lastSentEntry = reminderOffsets
                    .map(o => getReminderEntry(r.inv.id, o))
                    .filter(Boolean)
