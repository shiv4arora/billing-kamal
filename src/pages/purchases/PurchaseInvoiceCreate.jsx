...
import { buildInvoiceTotals, formatCurrency, nextInvoiceNumber, today, generateId } from '../../utils/helpers';
...
export default function PurchaseInvoiceCreate() {
  ...
  const { addPurchaseInvoice, updatePurchaseInvoice, getPurchaseInvoice, addStockEntry } = useInvoices();
  const { active: products, add: addProduct, update: updateProduct, updateStock } = useProducts();
  const { active: suppliers } = useSuppliers();
  const { settings, bumpPurchaseNo, bumpSkuNo } = useSettings();
  const { addPurchaseEntry, addPaymentOut } = useLedger();
  const isEdit = !!id;
  ...
  const handleSave = async (status) => {
    ...
    const invNo   = isEdit ? getPurchaseInvoice(id)?.invoiceNumber : nextInvoiceNumber(settings.invoice.purchasePrefix, bumpPurchaseNo());

    // Assign official sequential SKUs to new items that don't have a manual code
    const newItems = validItems.filter(i => i.isNew);
    const needsAutoSku = newItems.filter(i => !i.sku.trim());
    let autoSkuStart = needsAutoSku.length > 0 ? bumpSkuNo(needsAutoSku.length) : null;
    let autoSkuIdx = 0;
    // Also bump for items that already have a preview sku from local counter
    const hasPreviewSku = newItems.filter(i => i.sku.trim());
    if (hasPreviewSku.length > 0) {
      // Sync global counter past the highest preview SKU used
      const maxPreview = hasPreviewSku.reduce((max, i) => {
        const n = parseInt(i.sku); return isNaN(n) ? max : Math.max(max, n);
      }, 0);
      if (maxPreview >= (settings.nextSkuNo || 1001)) {
        bumpSkuNo(maxPreview - (settings.nextSkuNo || 1001) + 1);
      }
    }
    ...
    for (const item of validItems) {
      ...
      if (item.isNew) {
        const finalSku = item.sku.trim() || String(autoSkuStart + autoSkuIdx++);
        ...
        resolvedItems.push({ ...item, sku: finalSku, productId, productName: item.productName });
        continue;
    ...
    const invoiceTotals = buildInvoiceTotals(resolvedItems, false);

    const inv = {
      invoiceNumber: invNo, supplierInvoiceNumber: supplierInvNo,
      date, supplierId, supplierName: supplier?.name || '',
      items: invoiceTotals.items, ...invoiceTotals,
      amountPaid: paid, paymentMethod, paymentStatus: payStatus, notes, status,
    };

    if (isEdit) { updatePurchaseInvoice(id, inv); toast.success('Invoice updated'); navigate('/purchases'); return; }

    const saved = addPurchaseInvoice(inv);
    if (status === 'issued') {
      resolvedItems.forEach(item => {
        updateStock(item.productId, +item.quantity);
        addStockEntry({ productId: item.productId, date, movementType: 'purchase', quantity: +item.quantity, referenceId: saved.id, referenceNo: invNo });
      });
      // Auto-post to supplier ledger
      addPurchaseEntry({ supplierId, supplierName: supplier?.name || '', date, invoiceId: saved.id, invoiceNo: invNo, amount: invoiceTotals.grandTotal });
      if (paid > 0) {
        addPaymentOut({ supplierId, supplierName: supplier?.name || '', date, amount: paid, method: paymentMethod, referenceId: saved.id, referenceNo: invNo, narration: `Payment made against ${invNo} (${paymentMethod})` });
      }
    }
    toast.success(`${invNo} created${status === 'issued' ? ' · Stock & prices updated' : ''}`);
