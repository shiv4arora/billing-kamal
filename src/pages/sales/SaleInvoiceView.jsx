...
export default function SaleInvoiceView() {
  ...
  const { getSaleInvoice, updateSaleInvoice } = useInvoices();
  const { settings } = useSettings();
  const { addPaymentIn, addSaleReturn } = useLedger();
  ...
  const markPaid = () => {
    const rem = remaining;
    updateSaleInvoice(id, { paymentStatus: 'paid', amountPaid: inv.grandTotal, status: 'paid', paymentDate: today() });
    if (rem > 0.01) {
      addPaymentIn({ customerId: inv.customerId, customerName: inv.customerName, date: today(), amount: rem, method: inv.paymentMethod || 'cash', referenceId: id, referenceNo: inv.invoiceNumber, narration: `Full payment — ${inv.invoiceNumber}` });
    }
  };
  const voidInv = () => { if (confirm('Void this invoice?')) updateSaleInvoice(id, { status: 'void' }); };
  ...
  const handleRecordPayment = () => {
    ...
    updateSaleInvoice(id, { amountPaid: newPaid, paymentStatus: newStatus, paymentMethod: payForm.method, paymentDate: payForm.date });
    addPaymentIn({ customerId: inv.customerId, customerName: inv.customerName, date: payForm.date, amount: amt, method: payForm.method, referenceId: id, referenceNo: inv.invoiceNumber, narration: payForm.notes || `Payment received — ${inv.invoiceNumber} (${payForm.method})` });
  ...
  const handleSaleReturn = () => {
    ...
    addSaleReturn({ customerId: inv.customerId, customerName: inv.customerName, date: retForm.date, amount: +retForm.amount, referenceId: id, referenceNo: inv.invoiceNumber, narration: retForm.notes || `Sale Return — ${inv.invoiceNumber}` });
