...
export default function SaleInvoiceCreate() {
  ...
  const { addSaleInvoice, updateSaleInvoice, getSaleInvoice, addStockEntry } = useInvoices();
  ...
  const handleSave = (status) => {
    ...
    const invNo = isEdit ? getSaleInvoice(id)?.invoiceNumber : nextInvoiceNumber(settings.invoice.salePrefix, bumpSaleNo());
    const dueD = dueDate || (() => { const d = new Date(date); d.setDate(d.getDate() + settings.invoice.defaultDueDays); return d.toISOString().slice(0,10); })();

    const inv = { invoiceNumber: invNo, date, dueDate: dueD, customerId, customerName: customer?.name || '', customerPlace: customer?.place || '', customerType, items: totals.items, ...totals, amountPaid: paid, paymentMethod, paymentStatus: payStatus, paymentDate: paid > 0 ? today() : null, notes, status };

    if (isEdit) {
      updateSaleInvoice(id, inv);
      ...
      if (status === 'issued') {
        ...
        if (paid < totals.grandTotal) updateBalance(customerId, totals.grandTotal - paid);
        // Auto-post to customer ledger
        addSaleEntry({ customerId, customerName: customer?.name || '', date, invoiceId: saved.id, invoiceNo: invNo, amount: totals.grandTotal });
        if (paid > 0) {
          addPaymentIn({ customerId, customerName: customer?.name || '', date, amount: paid, method: paymentMethod, referenceId: saved.id, referenceNo: invNo, narration: `Payment received against ${invNo} (${paymentMethod})` });
        }
      }
      toast.success(`Invoice ${invNo} created`);
      setTimeout(() => navigate(`/sales/${saved.id}`), 300);
      return;
    }
    setSaving(false);
    navigate('/sales');
