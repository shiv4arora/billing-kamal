...
export default function CustomerLedger() {
  ...
  const { addPaymentIn, addSaleReturn, addAdjustment, getEntriesByParty, getBalance } = useLedger();
  ...
  const handlePayment = () => {
    ...
    addPaymentIn({ customerId: id, customerName: customer?.name || '', date: payForm.date,
  ...
  const handleReturn = () => {
    ...
    addSaleReturn({ customerId: id, customerName: customer?.name || '', date: retForm.date,
  ...
  const handleAdj = () => {
    ...
    addAdjustment({ partyType: 'customer', partyId: id, partyName: customer?.name || '',
      date: adjForm.date,
        ...
        <Card padding={false}>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {entries.length === 0 ? (
                  <tr><td colSpan="6" className="text-center py-12 text-gray-400">No transactions yet. Issue a sale invoice to auto-populate.</td></tr>
                ) : entries.map(e => (
